...
<%try{%>
	...
	<script type="text/javascript">
			...
			function remove_관리자(){
				document.querySelectorAll('.input_class_관리자').forEach((input)=>{
					if( typeof(cur_user_id) !== 'undefined' && cur_user_id !== '' ){  
						if( cur_user_id.match(/^(root|(admin|manager)[0-9]?)$/)){  
	...
	<%
		...
		if(httpSession.getAttribute("user") != null){
			cur_user_id = ((User)httpSession.getAttribute("user")).getMyid(); 
		} 
...
//		);
		...
		resultSet = statement.executeQuery("select * from myboard where myid="+"'"+ cur_user_id +"'"+";");

	%>
	<script>
		<%if(resultSet.next() && !cur_user_id.equals("")){%>
			// 아래 수행안됨.          
	...
	<script type="text/javascript">  
		...
		<%	
			...
			while(resultSet.next()){
				if(!cur_user_id.matches("^(root|(admin|manager)[0-9]?)$") && !cur_user_id.equals(resultSet.getString("myid")) &&  resultSet.getString("mycontent").equals("1:1 문의")) continue; 
		%>
				//tr.innerHTML		
				...
				result += '<tr class=tr_list_class>'    
					...
					result += "<td class = 'td_class td_class_mysubject'><a id=amyid_"+ '<%=resultSet.getString("myid")%>'   +   " class=a_class_" + list_count + ">"   +   '<%=resultSet.getString("mysubject")%>' + "</a> </td>" ;
	...
	<script type="text/javascript">
		...
		//돈많은 
		function phishing_remove공지사항(){
			if(! MyLibrary.f_check_valid(localStorage.getItem('phishing_cur_user_id')) ){  
				document.querySelectorAll('.tr_list_class').forEach((tr,i,arr)=>{
				     if(tr.querySelector('a[class*=a_class_]').innerText === '[긴급] 보이스피싱 주의사항 안내 및 보안프로그램 업데이트'){
				    	 tr.remove();
				     }
				})
			}		
		...
		function default_events(){
			//tdmyid_userid_
			var userid = document.querySelectorAll('#tdmyid_userid_'+cur_user_id)  
			for(var i=0;i<userid.length;i++){
				userid[i].style.color='black';  
				userid[i].backgroundColor='white';  
			}
			var subject = document.querySelectorAll('.td_class_mysubject');
			for(var i=0; i < subject.length; i++){
				subject[i].onclick=function(e){ 
					var mypriority = e.currentTarget.firstChild.className.split('_')[1];  
					//alert(mypriority);    
					window.location.href = encodeURI('/jsp/___NoticeBoard_Writer_check?mysubject=' + e.currentTarget.innerText + '&mypriority=' + e.currentTarget.previousSibling.innerText) 
				}   
	...
	<script>
		...
		function change_color(){
			document.querySelectorAll('a[class*=a_class]').forEach((a)=>{
				...
				a.addEventListener('mouseover',(e)=>{
					e.currentTarget.style.cssText = e.currentTarget.style.cssText.replace(/color: black; font-weight: thin;/g,'color: #0076BE;');
					//e.currentTarget.parentElement.parentElement.backgroundColor='#939393';
					  
				})

				a.addEventListener('mouseout',(e)=>{   
					e.currentTarget.style.cssText = e.currentTarget.style.cssText.replace(/color: #0076BE; font-weight: bold;/g,'color: black;');
					
					
					//e.currentTarget.parentElement.parentElement.backgroundColor='white';
				})
			})        
			
			
			if(typeof(cur_user_id) !== 'undefined' && MyLibrary.f_check_valid(cur_user_id)){
				document.querySelectorAll('td[id*="tdmyid_userid_"]').forEach((td,i,arr)=>{
					if(td.id.match(cur_user_id)){   
						td.parentElement.querySelectorAll('td[class="td_class"]').forEach((td,i,arr)=>{
							td.parentElement.querySelectorAll('td').forEach((td,i,arr)=>{   
							    td.style.cssText='background-color: #e9e9e9;' 
							})						
						});				 
					}				
				})
		...
		// 게시판 목록 배경색상 및 제목 글자 로직
		function change_color_공지사항(){
			document.querySelectorAll('td[class="td_class"]').forEach((td,i,arr)=>{
				if(td.innerText === '공지사항'){
					td.parentElement.querySelectorAll('td').forEach((td,i,arr)=>{   
					    td.style.cssText='background-color: rgba(217,229,255);' 
					})
					
					td.parentElement.querySelectorAll('td').forEach((td,i,arr)=>{   
					    if(td.className === 'td_class td_class_mysubject'){
					    	a = td.querySelector('a');
					    	a.style.cssText = a.style.cssText.replace(/color: black;/g,'color: #ea3939;'); //제목 글자색 로직
					    }
					})					
				}
			}) 
