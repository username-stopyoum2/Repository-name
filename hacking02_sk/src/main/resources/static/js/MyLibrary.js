...
var MyLibrary = {
	...
	RaonSecure:{
		Nxkey: {
			init: () => {
				...
				/*
				MyLibrary.f_script_wirte_s(['https://localhost:8080/MyLibraryAndTools/my_js/파일.js','/MyLibraryAndTools/my_js/파일.js','파일.js'])
				*/ 
	...
	*/			
				...
				nxkey_param = {
					method: "POST",
					url: "/RaonSecure",      //protocol://hostname:port/pathname?name=value&searchparam#hash
				...
				if( typeof(param) === 'undefined' || !MyLibrary.f_check_valid(param)) {
					console.log(`{
						tryp: "nxkey",
						url: "/RaonSecure",
						TNK_SR:"true"
				...
				if(typeof(param.url) !== 'undefined' && MyLibrary.f_check_valid(param.url)){
					nxkey_param.url = param.url
				}


								
				if(typeof(param.type) !== 'undefined' && MyLibrary.f_check_valid(param.type)){
					nxkey_param.send_querystring = 'type=' + param.type;     					
				}				
				
				
				if(typeof(param.TNK_SR) !== 'undefined' && MyLibrary.f_check_valid(param.TNK_SR)){
					nxkey_param.send_querystring += '&TNK_SR=' + param.TNK_SR;      					
				}				
				

				nxkey_param.done_2xx_callback = ()=>{ 
					
					window.TNK_SR = JSON.parse(window.xmlHttpRequest.response).TNK_SR;  	
					if(MyLibrary.f_check_valid(window.TNK_SR)) {
						//alert(`window.TNK_SR = ${window.TNK_SR}`);
						localStorage.setItem('TNK_SR',window.TNK_SR);
						return window.TNK_SR;
					}else{
						alert(`window.TNK_SR 값 없음.`)
						return `window.TNK_SR 값 없음.`
					}
				}
				
				
				
				xmlHttpRequest = MyLibrary.Base.xmlHttpRequest( nxkey_param ) 
			},
			
			setInstallPageAndTK_Loading: (param)=>{
...
이후에 호출해줘야 됨.					
				...
				*/				
				nxkey_param = {
					url: TouchEnNxConfig.installPage.nxkey
				}	

				if( typeof(param) === 'undefined' || !MyLibrary.f_check_valid(param)) {
					console.log(`{ url: ${TouchEnNxConfig.installPage.nxkey}} 기본설정으로 잡힘.`);
				}
			...
			setForm: (param)=>{
				window.addEventListener('load',(e)=>{
					alert('asfd');
					if(!TouchEnNxConfig.onload) TK_Loading(); 
				// TK_Loading() 을 2번 호출되면 정상복호화 되거나 -1 오류 뜸. TK_makeEncData(form) 할 시 내부적으로 TK_Loading() 함? 떄문에 body.onload 떄 호출 x
				})				
			},
			setInputs: (param)=>{
				/* 
					{
						...
						id: 'input_id', // 해당 input에 id가 없을 시 input_id_인덱스 형식으로 설정됨.
						name: 'input_name', // name 없을 시 input_name_인덱스 형식으로 설정됨.
						data_enc: 'on', // 없을 시 설정됨.
						data_dataType: 'Aan@'
					}
				
				*/
				if(MyLibrary.f_check_valid(param) ){
					if(MyLibaray.f_check_valid(param.inputs)){
						inputs.forEach((input,i,arr)=>{
							if( !MyLibrary.f_check_valid(input.id) && MyLibrary.f_check_valid(param.id)) input.id = param.id+'_'+i
							if( !MyLibrary.f_check_valid(input.name) && MyLibrary.f_check_valid(param.name)) input.name = param.name + '_' + i
							if( !MyLibrary.f_check_valid(input.getAttribute('data-enc')) && MyLibrary.f_check_valid(param.data_enc)) input.setAttribute('data-enc',param.data_enc);
							if( !MyLibrary.f_check_valid(input.getAttribute('data-dataType')) && MyLibrary.f_check_valid(param.data_dataType)) input.setAttribute('data-dataType',param.data_dataType);
							if( !MyLibrary.f_check_valid(input.type) ) input.type = 'password'
						})						
					}else{
						
					}
					
					if(MyLibaray.f_check_valid()){
						
					}else{
						
					}
					if(MyLibaray.f_check_valid()){}else{}
					if(MyLibaray.f_check_valid()){}else{}
					if(MyLibaray.f_check_valid()){}else{}
					if(MyLibaray.f_check_valid()){}else{}
					if(MyLibaray.f_check_valid()){}else{}
